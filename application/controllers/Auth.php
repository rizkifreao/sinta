...
class Auth extends CI_Controller {
    ...
    function edit_user($id) {
        ...
        if (!$user) {
            $this->session->set_flashdata('alert', error("Pengguna tidak ditemukan !"));
            redirect('auth/member', 'refresh');
        }
        $groups = $this->ion_auth->groups()->result_array();
        ...
        if (isset($_POST) && !empty($_POST)) {
            ...
            if ($this->form_validation->run() === TRUE) {
                ...
                $data = array(
                    'first_name' => $this->input->post('first_name'),
                    'last_name' => $this->input->post('last_name'),
                    'id_konsumen' => ($groupData != null) ? ($groupData == 1) ? 0 : $this->input->post('company') : 2 ,
                    'phone' => $this->input->post('phone'),
                    'username' => $this->input->post('username'),
                );
    ...
    function edit_profil($id) {
        ...
        $this->template->display('auth/edit_profil', $this->data);
    }
